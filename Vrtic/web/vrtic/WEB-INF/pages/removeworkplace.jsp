...
<html>
  ...
  <body>
    ...
    <!-- TOP BAR -->
  	...
  	<div class="row">
  	  <div class="twelve columns">
	    <h1>Brisanje objekta</h1>
			 
	    <!-- Children list from selected group --> 
	    <c:choose>
	      <c:when  test="${workplaces != null && workplaces.isEmpty() == false}">
	        <p>Odaberite objekt sa liste koji želite obrisati</p>
	        <form method="post">
	          <select name="workplace"> 
			 	<c:forEach var="workplace" items="${workplaces}">
				  <option value="${workplace.id}">
				    <c:out value="${workplace.address}" />, <c:out value="${workplace.town}" /> 
				  </option>
				</c:forEach>
			  </select>
	
		      <input class="button" type="submit" name="metoda" value="Obriši">
	        </form>
	      </c:when>
	      <c:otherwise>
	        <p>Trenutno nema objekta</p>
	      </c:otherwise>
	    </c:choose>  
