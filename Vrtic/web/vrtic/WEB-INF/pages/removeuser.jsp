...
<html>
  ...
  <body>
    <!-- TOP BAR -->
  	...
  	<div class="row">
  	  <div class="twelve columns">	
	    <h1>Izbrišite korisnika</h1>
	    
	    <c:choose>
	      <c:when  test="${users != null}">
	      <p>Odaberite korisnika sa liste kojeg želite obrisati.</p>
	      <form method="post">
		      <c:forEach var="user" items="${users}">
		        <input type="radio" name="users" value="<c:out value="${user.nick}" />">
		        <label><c:out value="${user.firstName}" /> <c:out value="${user.lastName}" /> <c:out value="${user.nick}" /></label>
		        <br>
		      </c:forEach>
	
		      <input class="button" type="submit" name="metoda" value="Izbriši">
	        </form>
	      </c:when>
	      <c:otherwise>
